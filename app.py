...
import pickle
import logging
logging.basicConfig(filename="logging_notebook.txt",filemode='a',format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',)
...
try:
    ...
    logging.error('Error in loading the data,  could be streamlit import error')
...
try:
    ...
    logging.error('Error in EDA section maybe container not defined')
...
try:
    ...
    logging.error('Error in loading the model')
